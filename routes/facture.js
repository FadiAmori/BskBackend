...
router.post('/', async (req, res) => {
  try {
    ...
    const { numeroFacture, client, liste, ...factureData } = req.body;
    ...
    const facture = new Facture({ ...factureData, client, liste, numeroFacture: newNumero });
    await facture.save();
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const { numeroFacture, client, liste, ...updateData } = req.body;
    ...
    const facture = await Facture.findByIdAndUpdate(req.params.id, { ...updateData, client, liste }, { new: true })
      .populate('client')
      .populate('liste.produit');
    if (!facture) return res.status(404).json({ error: 'Facture not found' });
    res.json(facture);
